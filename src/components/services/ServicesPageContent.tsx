      ...
      <section className="section-padding bg-warm-white">
        <div className="container-wide space-y-16 sm:space-y-20 md:space-y-24">
          {services.map((service, i) => {
            ...
            return (
              <div key={service.id} className={`flex flex-col ${i % 2 !== 0 ? 'lg:flex-row-reverse' : 'lg:flex-row'} gap-8 sm:gap-12 lg:gap-20 items-center`}>
                ...
                <FadeIn className="w-full lg:w-1/2" delay={0.2}>
                  ...
                  <h3 className="font-serif text-brand font-semibold mb-4 sm:mb-6" style={{ fontSize: 'clamp(1.5rem, 1.5vw + 0.75rem, 2.25rem)' }}>
                    {headline}
                  </h3>
                  <p className="text-text-muted leading-relaxed mb-6 sm:mb-8 text-base sm:text-lg">
                    {description}
                  </p>

                  <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-8 sm:mb-10">
                    {currentStats.map((stat, idx) => (
                      <div key={stat._id || `${stat.label}-${idx}`} className="bg-white p-3 sm:p-4 border border-stone/30 rounded-sm">
                        <p className="font-serif text-xl sm:text-2xl font-bold text-brand">{stat.value}</p>
                  ...
                  <Link
                    ...
                    className="inline-flex items-center gap-2 px-5 sm:px-7 py-3 sm:py-3.5 bg-brand text-white text-xs sm:text-sm font-semibold uppercase tracking-wider rounded-sm cursor-pointer hover:bg-gold transition-colors shadow-sm"
                  >
