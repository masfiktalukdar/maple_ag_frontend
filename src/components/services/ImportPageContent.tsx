...
import { FaCogs, FaBolt, FaWrench, FaTools, FaCar, FaGlobe } from "react-icons/fa";
...
// Icon Mapping for Import Categories
const getIcon = (itemName: string) => {
  const name = itemName.toLowerCase();
  if (name.includes("machine") || name.includes("industrial")) return <FaCogs size={20} />;
  if (name.includes("electronic") || name.includes("power")) return <FaBolt size={20} />;
  if (name.includes("part") || name.includes("component")) return <FaWrench size={20} />;
  if (name.includes("equipment")) return <FaTools size={20} />;
  if (name.includes("auto")) return <FaCar size={20} />;
...
export default function ImportPageContent({ products: importProducts, networkCategory, partners = [], categoryItems = [] }: ImportPageContentProps) {
  ...
  }, [selectedCategory, importProducts]);
    ...
    <>
      ...
      <section className="relative pt-32 pb-24 min-h-[500px] flex items-center">
        ...
        <div className="container-wide relative z-10 text-center">
          <FadeIn>
            <span className="eyebrow text-accent block mb-4">SERVICE: IMPORT</span>
        ...
        <div className="container-wide">
          <FadeIn>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5 mt-10 max-w-5xl mx-auto">
              {(categoryItems.length > 0 ? categoryItems : service.items.map(title => ({ title, imageUrl: 'https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?auto=format&fit=crop&w=600&q=80' }))).map((item: any, i: number) => (
                <div
                ...
                >
                  <div className="relative h-44 sm:h-48 w-full bg-stone-100 overflow-hidden">
                    <SafeImage
                      src={item.imageUrl || item.image || "https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?auto=format&fit=crop&w=600&q=80"}
