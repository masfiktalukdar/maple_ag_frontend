...
import { supplyProducts, type Product } from "@/data/products";
import SectionHeader from "@/components/shared/SectionHeader";
import CTABanner from "@/components/shared/CTABanner";
import ProductCard from "@/components/shared/ProductCard";
import Modal from "@/components/shared/Modal";
import SearchableDropdown from "@/components/shared/SearchableDropdown";
import { FaWarehouse, FaSnowflake, FaClipboardList, FaTruck, FaProjectDiagram, FaMapMarkedAlt, FaCube } from "react-icons/fa";
...
const getIcon = (itemName: string) => {
  ...
  if (name.includes("warehous") || name.includes("storage")) return <FaWarehouse size={20} />;
  if (name.includes("cold")) return <FaSnowflake size={20} />;
  if (name.includes("inventory") || name.includes("stock")) return <FaClipboardList size={20} />;
  if (name.includes("transport") || name.includes("truck")) return <FaTruck size={20} />;
  if (name.includes("distribution") || name.includes("network")) return <FaProjectDiagram size={20} />;
  if (name.includes("last-mile") || name.includes("delivery")) return <FaMapMarkedAlt size={20} />;
    ...
    <>
      ...
      <section className="relative pt-32 pb-24 min-h-[500px] flex items-center">
        ...
        <div className="container-wide relative z-10 text-center">
          <FadeIn>
            <span className="eyebrow text-gold block mb-4">SERVICE: SUPPLY & DISTRIBUTION</span>
        ...
        {submitted ? (
          <div className="text-center py-12">
            ...
            <p className="text-text-muted">
              Thank you for your interest in {selectedProduct?.name}. Our distribution specialists will contact you shortly.
            </p>
            <button
          ...
          <form onSubmit={handleFormSubmit} className="space-y-4">
            <p className="text-sm text-text-muted mb-6">
              Please provide the details below so we can arrange the supply of {selectedProduct?.name}.
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-medium text-navy mb-1 uppercase tracking-wider">Delivery Location</label>
                <input required type="text" placeholder="e.g., Factory Address" className="w-full bg-ivory border border-stone/40 px-4 py-2.5 rounded-sm focus:outline-none focus:border-terracotta" />
              </div>
              <div>
                <label className="block text-xs font-medium text-navy mb-1 uppercase tracking-wider">Delivery Frequency</label>
                <select className="w-full bg-ivory border border-stone/40 px-4 py-2.5 rounded-sm focus:outline-none focus:border-terracotta appearance-none">
                  <option>Spot / One-time</option>
            ...
            <div>
              <label className="block text-xs font-medium text-navy mb-1 uppercase tracking-wider">Quality Specifications</label>
              <textarea rows={3} placeholder="Provide specific grade or quality parameters" className="w-full bg-ivory border border-stone/40 px-4 py-2.5 rounded-sm focus:outline-none focus:border-terracotta resize-none"></textarea>
            </div>
